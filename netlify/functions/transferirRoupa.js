...
exports.handler=async(event,context)=>{
  ...
  try{
    ...
    console.log(JSON.stringify({at:'transferirRoupa.incoming',version:VERSION,lenRaw:rawBytes,baseKB:Math.round(imgA.buf.length/1024),garmentKB:Math.round(imgB.buf.length/1024),mimes:[imgA.mime,imgB.mime],ratio:ratioStr,reqId:(context&&context.awsRequestId)||null}));
    const image=stubTinyPng(); const durationMs=Date.now()-started;
    console.log(JSON.stringify({at:'transferirRoupa.done',version:VERSION,durationMs,reqId:(context&&context.awsRequestId)||null}));
